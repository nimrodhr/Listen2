...
from listen.knowledge.preprocessing import preprocess_documents
...
def load_document(file_path: str, preprocess: bool = True) -> list[Document]:
    """Load a single document file and return LangChain Documents.

    Args:
        file_path: Path to the file to load.
        preprocess: If True, clean the text (strip PDF noise, normalize whitespace).
    """
    path = Path(file_path)
    ...
    try:
        ...
        # Preprocess: clean noise from extracted text
        if preprocess:
...
def load_directory(directory: str, preprocess: bool = True) -> list[Document]:
    """Load all supported documents from a directory."""
    all_docs = []
    for file_path in scan_directory(directory):
        docs = load_document(str(file_path), preprocess=preprocess)
